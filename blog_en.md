...
Have you noticed that Large Language Models (LLMs) often exhibit a kind of “expression lag”?

More specifically, if we decompose LLM’s capabilities, we usually see three distinct levels:

Given a policy model $\pi$, a task ($T$), and a verifier ($V$) that checks whether the task is solved, we have

1. **Task Performance (TP, $P_{TP}(\pi) \triangleq \Pr_{A\sim\pi(\cdot| T)}[V(T,A)=1]$)**: Ask LLMs to solve a problem—they might get it wrong.
2. **Self-Verification (SV, $P_{SV}(\pi,A,s) \triangleq \Pr[SV_s(T,A)=V(T,A)]$, where $s$ is some verification strategy)**: Even when LLMs answer incorrectly, if you ask them to check their own work, they can often say “this isn’t right.” This suggests its **language-based verification ability (SV)** can be stronger than its **execution ability (TP)**.
3. **Representation Readout (RR, $P_{RR}(\pi,A,l,g) \triangleq \Pr[ RR_l(T,A)=V(T,A) ]$, where $RR_l(T,A) \triangleq g(h_l(T,A))$ is a monitor $g$ trained on layer-$l$ representations)**: If we bypass language output and probe internal activations directly, we may find LLMs “internally” encodes features that more clear to judge and monitor.

A growing body of phenomenons support an inequality as follow:
...
|                  ![loop](./loop.svg)                  |
| :---------------------------------------------------: |
| A gap often exists among the three levels in practice |

In other words, the model can be a paradox: **“it knows” internally (RR), but “can’t quite say it” in language (SV), which leads to “can’t reliably do it” (TP).** Chain-of-thought and related reasoning techniques sketch a path for $P_{TP}$ to catch up to $P_{SV}$. **But who can bridge the gap between $P_{SV}$ and $P_{RR}$?** Why can’t deep intuition be perfectly translated into explicit language?

This is exactly what we aim to explore.

To narrow the gap between representational “intuition” and language-level “expression,” we turn to **Looped Transformers**.
...
>
> If Chain-of-Thought (CoT) can be viewed as scaling **sequence length** for more explicit thinking steps, and traditional scaling laws are considered as scaling **parameters and data scope** for stronger capabilities, Looped Transformers open another scaling axis: **computation depth via recurrence**.
>
> The key idea is **recursive processing of internal representations**. Instead of eagerly decoding intermediate features into discrete tokens, the model feeds its internal representations back through the *same* set of weights for multiple loops.
>
...
>
> Current explorations broadly focus on two questions: **how to loop** and **where /when to loop**.
>
> **How to loop**
>
...
> **Where/when to loop?**
>
> > Not every token deserves deep thought. Stop-words like “the” or “is” don’t need it, while key logical pivots might.
>
...
This hints at a harsh possibility: **looping may organize expression while also compressing/eroding the “raw” internal signal.** If “ too much latent thinking” dulls the sharpness of “intuition,” is it worth it? This is something we must take seriously.
...
- **Only in the final loop** does the injected concept become identifiable by the model.
